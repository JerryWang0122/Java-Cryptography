...
public class DigitalSignatureCreator {
    ...
    public static void main(String[] args) throws Exception {
        // 小王
        // 合約檔位置
        String contractPath = "src/main/java/security/sign/my_contract.txt";
        // 公鑰檔位置
        String publicKeyPath = "src/main/java/security/sign/publicKey.key";
        // 私鑰檔位置
        String privateKeyPath = "src/main/java/security/sign/privateKey.key";
        // 數位簽章檔位置
        String signaturePath = "src/main/java/security/sign/signature.sig";
        ...
        // 數位簽章: 將私鑰(個人印章) 蓋在合約(my_contract.txt)上
        byte[] digitalSignature = KeyUtil.signWithPrivateKey(privateKey, contractPath);
        // 保存數位簽章(signature.sig)
        KeyUtil.saveSignatureToFile(digitalSignature, signaturePath);

        System.out.printf("小王的公鑰(Base 64): %s%n", Base64.getEncoder().encodeToString(publicKey.getEncoded()));
        System.out.printf("小王的數位簽章(Base 64): %s%n", Base64.getEncoder().encodeToString(digitalSignature));
        System.out.println();
        System.out.printf("小王的公鑰(HEX): %s%n", KeyUtil.bytesToHex(publicKey.getEncoded()));
        System.out.printf("小王的數位簽章(HEX): %s%n", KeyUtil.bytesToHex(digitalSignature));
